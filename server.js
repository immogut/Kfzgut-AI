...
const mails = {
  ...
  trialWarning: (name, days) => ({
    ...
    html: mailBase(`
      ...
      <p style="color:#5a6478;font-size:15px;line-height:1.75;margin:0 0 20px">Hallo ${name},<br><br>
      Ihr kostenloser Testzeitraum endet in ${days} ${days === 1 ? 'Tag' : 'Tagen'}. Um weiterhin alle Funktionen zu nutzen, abonnieren Sie jetzt ab 99 €/Monat zzgl. USt..</p>
      <div style="text-align:center;margin:24px 0">
  ...
  trialExpired: (name) => ({
    ...
    html: mailBase(`
      ...
      <div style="text-align:center;margin:24px 0">
        <a href="${process.env.FRONTEND_URL}/konto.html" style="display:inline-block;background:#1a56a0;color:white;padding:14px 32px;border-radius:9px;text-decoration:none;font-size:15px;font-weight:500">Jetzt abonnieren – ab 99 €/Monat →</a>
      </div>
      <p style="color:#9aa0ae;font-size:12px;margin:16px 0 0">Mit freundlichen Grüßen<br><strong style="color:#1e2433">KfzGut-AI</strong></p>`)
  }),
  paymentFailed: (name) => ({
...
app.post('/api/checkout', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Tarif: 'solo' (1 Nutzer, 99 €) oder 'team' (bis 3 Nutzer, 149 €)
...
app.get('/api/admin/stats', adminMiddleware, async (req, res) => {
  ...
  // Umsatz nach Tarif (netto): Büro (mehr als 1 Platz) 149 €, Einzelplatz 99 €
  const revenue = payingRows.reduce((sum, r) => sum + ((r.plan_seats || 1) > 1 ? 149 : 99), 0);
